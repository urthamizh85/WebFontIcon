...
for dirname, dirnames, filenames in os.walk(INPUT_SVG_DIR):
  for filename in filenames:
    ...
    if ext in ['.svg', '.eps']:
      ...
      chr_code = None
      for cnsicon in manifest_data['icons']:
        if cnsicon['name'] == name:
          chr_code = cnsicon['code']
          break
  ...
  fontfile = '%s/cnsicons' % (OUTPUT_FONT_DIR)

build_hash = m.hexdigest()
print  build_hash
print manifest_data.get('build_hash')
if build_hash == manifest_data.get('build_hash'):
  ...
  #subprocess.call('move/Y ' + fontfile + '.eotlite ' + fontfile + '.eot', shell=True)

  # Hint the TTF file
  subprocess.call('ttfautohint -s -f -n ' + fontfile + '.ttf ' + fontfile + '-hinted.ttf > /dev/null 2>&1 && move/Y ' + fontfile + '-hinted.ttf ' + fontfile + '.ttf', shell=True)
